...
export class UsersDetailsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.darkThemeSubscription = this.sharedService.isDarkMode.subscribe(
      (res) => (this.isDarkMode = res),
    );
    localStorage.getItem('theme') === 'dark'
      ? (this.isDarkMode = true)
      : (this.isDarkMode = false);

    this.currencyCode = JSON.parse(localStorage.getItem('user')).country[
      'currencyCode'
    ];
    this.oldCurrencyCode = JSON.parse(localStorage.getItem('user')).country[
      'currencyCode'
    ];

    this.loadTableObservableSubscription =
      this.authService.loadTableObservable.subscribe((res) => {
        console.log(res);
        this.loadUserdetailsInTable();
      });
  }
  ...
  convertAmount(baseAmount?: number): number {
    if (this.selectedCurrency && exchangeRates[this.selectedCurrency.code]) {
      ...
      return this.convertExpenseToselectedCurrency(
        baseAmount,
  ...
  convertExpenseToselectedCurrency(
    amount: number,
    ...
    const convertedAmount =
      amountToINR / exchangeRatesToINR[resultcurrencyCode];
  ...
  addOrUpdateUser(id?: string) {
    ...
    if (id) {
      ...
      this.http
        .patch(
          `https://travektrail-app-default-rtdb.firebaseio.com/users/${id}.json`,
          userToUpdate,
        )
        .subscribe((res) => {
          // console.log(res);
  ...
  openEditDialog(userId: string) {
    ...
    this.http
      .get(
        `https://travektrail-app-default-rtdb.firebaseio.com/users/${userId}.json`,
      )
      .subscribe((user: User) => {
        ...
        this.profileImage = user.profileImage
          ? user.profileImage
          : `https://ui-avatars.com/api/?name=${user.firstName}+${user.lastName ? user.lastName : ''}&bold=true`;
        ...
        this.addUserForm1.patchValue({
          state: this.filteredstates.find(
            (state) => state.code === this.userToEdit.state,
          ),
        });
        // console.log(this.addUserForm1);
      });
  ...
  showDeleteUser(id: string) {
    ...
    if (id === JSON.parse(localStorage.getItem('user')).id) {
      this.messageService.add({
        severity: 'warn',
        summary: 'Warn',
        detail: "Admin can't delete himself!.",
      });
      return;
    }
  ...
  deleteUser() {
    ...
    this.http
      .get(
        `https://travektrail-app-default-rtdb.firebaseio.com/users/${this.userId}.json`,
      )
      .subscribe((res) => {
        // console.log(res);
        let email = res['email'];
        ...
        this.http
          .post(
            'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyA0CxBGkhNtT1q48ZjBhu4aFn1FsQgYoxs',
            {
              email: email,
  ...
  deleteAuthUser(tokenId: string) {
    ...
    this.http
      .post(
        'https://identitytoolkit.googleapis.com/v1/accounts:delete?key=AIzaSyA0CxBGkhNtT1q48ZjBhu4aFn1FsQgYoxs',
        idToken,
      )
      .subscribe((res) => {
        // console.log(res);
  ...
  goToPage() {
    if (this.usersDetailTable && this.goToPageNumber > 0) {
      ...
      if (
        firstRowIndex >= 0 &&
        firstRowIndex < this.usersDetailTable.value.length
      ) {
        ...
        this.messageService.add({
          severity: 'warn',
          summary: 'Warn',
          detail: 'Invali page number entered!.',
        });
